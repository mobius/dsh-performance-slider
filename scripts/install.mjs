#!/usr/bin/env node
/*
 * Install dsh-performance-slider into the default dsh web profile.
 *
...
function fail(message) {
  console.error(`[dsh-performance-slider] ${message}`);
  process.exitCode = 1;
}
...
spawnSync(pnpm, ['--dir', profileDir, 'remove', 'dsh-performance-slider'], {
  stdio: 'ignore',
  env: process.env,
});
...
if (!patch.includes('name: dsh-performance-slider')) {
  ...
  console.log(`[dsh-performance-slider] patched ${patchPath}`);
} else {
  console.log(`[dsh-performance-slider] ${patchPath} already contains the row`);
}

console.log('[dsh-performance-slider] installed. Restart dsh web to load the slider.');
